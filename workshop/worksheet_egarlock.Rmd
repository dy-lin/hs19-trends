...
```{r worldmap}
pl <- ggplot() + 
  geom_polygon(data = FILL_THIS_IN, aes(x = FILL_THIS_IN, y = lat, group=FILL_THIS_IN,fill =(FILL_THIS_IN),text=FILL_THIS_IN))+
  #geom_point(data=data,aes(x = long, y = lat),alpha=0.5,size=0.5,colour="grey")+
  coord_fixed(1.3)+
  scale_fill_viridis()+
  theme_void()

ggplotly(FILL_THIS_IN,tooltip = FILL_THIS_IN)
...
```{r}
pl <- ggplot() + 
  geom_FILL_THIS_IN(data = FILL_THIS_IN, aes(x = long, y = lat, group = group, fill = FILL_THIS_IN(FILL_THIS_IN),text=paste(FILL_THIS_IN,Count, sep=";"))) +
  geom_FILL_THIS_IN(data=points_europe,aes(x=X,y=Y,text=str_wrap(affiliation,50)),alpha=0.5,size=0.5,colour="grey")+
  coord_fixed(1.3)+
  scale_fill_viridis()+
  theme_void()

FILL_THIS_IN(pl,tooltip=FILL_THIS_IN)
```
...
visualize_bigrams <- function(df_name, textfield, topic_title){
    ...
    pl <- graph_hold %>%
      ggraph(layout = FILL_THIS_IN) +
      geom_FILL_THIS_IN(aes(edge_colour = log(FILL_THIS_IN)), show.legend = TRUE) +
      geom_FILL_THIS_IN(aes(color = Term_Frequency, size = Term_Frequency), alpha = 0.7) +
      scale_fill_viridis_FILL_THIS_IN() +
      scale_edge_color_viridis(direction=-1)+
      geom_node_text(aes(label = name), repel = TRUE) +
      scale_color_viridis_FILL_THIS_IN(direction = -1) +
      theme_void() +
      guides(size=FALSE) +
      labs(title = FILL_THIS_IN(FILL_THIS_IN)) +
      theme(plot.title = element_text(size = 26, face = "bold"))
    
    ggsave(pl,filename = paste0("../workshop/", "bigrams_", str_to_lower(str_replace(topic_title, "\\s", "_")), ".png")
           ,width = 12
           ,height = 8)
    pl
}
...
```{r}
df_FILL_THIS_IN <- df %>% 
  filter(topic == "FILL_THIS_IN")
visualize_bigrams(df_FILL_THIS_IN, FILL_THIS_IN, "")
```
